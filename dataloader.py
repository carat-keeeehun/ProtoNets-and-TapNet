...
from utils import MiniImageNetDataset, TieredImageNet, get_data_dir
...
def get_dataloader(config, *modes):
    res = []
    if 'train' in modes[0]:
    ...
    for mode in modes:
        if config.data.dataset == 'miniImageNet':
            ...
            dataset = TieredImageNet(mode)

        if 'train' in mode:
            classes_per_it = config.training.classes_per_it_tr
            num_support = config.training.num_support_tr
            num_query = config.training.num_query_tr
            episodes = config.training.episodes_tr
        else:
            classes_per_it = config.training.classes_per_it_val
            num_support = config.training.num_support_val
            num_query = config.training.num_query_val
            episodes = config.training.episodes_val

        # print(f"dataset.y : {type(dataset.y)}")
        # print(f"dataset.y : {dataset.y.dtype}")
        # print(f"dataset.y : {dataset.y.type()}")
